#python not_neighbor_set.py <following.err> <followers.err> <AllUser> <following> <followers> <UserList> <OutputDirectory>
...
#隣接していないユーザの取得
def NotNeighborUsers(user, UserList, EdgeSet):
    NotNeighborList = []
    for temp in UserList:
        if(temp != user):
            if(not({(user, temp)} <= EdgeSet or {(temp, user)} <= EdgeSet)):
                NotNeighborList.append(temp)
    return NotNeighborList
...
while(count < 1000):
    ...
    not_neighbor = NotNeighborUsers(user1, tempList, EdgeSet)
    if(len(not_neighbor) == 0):
        if(user1 in tempList):
            tempList.remove(user1)
        continue
    else:
        user2 = random.choice(not_neighbor)
        if (user2 < user1):
            temp = user1
            user1 = user2
            user2 = temp
...
with open(argv[7]+"not_neighbor_set.txt", "w") as f:
